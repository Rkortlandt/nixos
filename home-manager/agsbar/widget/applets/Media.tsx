...
function MediaPlayer({ player }: { player: AstalMpris.Player }) {
  ...
  return (
    <box
      ...
      css={coverArtStyle}
      visible={createBinding(player, "title").as((t) => !!t && t.length > 0)}
    >
...
function BarMediaPlayer({ player, onClick }: { player: AstalMpris.Player; onClick: () => void }) {
  ...
  const title = createBinding(player, "title").as((t) =>
    t && t.length > 28 ? t.slice(0, 28).trim() + "..." : t || "Unknown Track"
  );

  const [showPlayIcons, setShowPlayIcons] = createState(false);

  return (
    <Gtk.Box class="bar-media-player" hexpand={false} halign={Gtk.Align.END}>
      <Gtk.EventControllerMotion
        onEnter={() => setShowPlayIcons(true)}
        onLeave={() => setShowPlayIcons(false)}
      />
      <box>
        <image class="bar-music-icon" pixelSize={22} iconName="Music" />
        <revealer revealChild={showPlayIcons} transitionType={Gtk.RevealerTransitionType.SLIDE_RIGHT}>
          <box>
            <button
              ...
              visible={createBinding(player, "canGoPrevious")}
            >
              <image class="bar-control-icon" pixelSize={22} iconName="Skip-Backward" />
            </button>
            <button
              class="no-bg"
              onClicked={() => player.play_pause()}
              visible={createBinding(player, "canControl")}
            >
              <image class="bar-control-icon bar-play-icon" pixelSize={22} iconName={playIcon} />
            </button>
            <button
              class="square-button-no-rad no-bg"
              onClicked={() => player.next()}
              visible={createBinding(player, "canGoNext")}
            >
              <image class="bar-control-icon" pixelSize={22} iconName="Skip-Forward" />
            </button>
          </box>
        </revealer>
  ...
  return (
    <box>
      <For each={players}>
        {(player, index) => {
          const onClick = () => {
            const numberOfPlayers = players.get().length;
            const current = shownPlayer.get();
            const newValue = current < numberOfPlayers - 1 && numberOfPlayers !== 1 ? current + 1 : 0;
            setShownPlayer(newValue);
          };

          return (
            <revealer
              revealChild={shownPlayer.as((number) => number === index.get())}
